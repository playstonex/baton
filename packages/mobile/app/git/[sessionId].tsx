  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [s, l, b] = await Promise.all([
        ...
        gitService.log(projectPath, 15).catch(() => null),
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: Spacing.md }}>
          <Pressable onPress={() => router.back()} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            <Text style={[Typography.footnote, { color: Colors.primary[500], fontWeight: '500' }]}>{'\u2190'} Back</Text>
          </Pressable>
        ...
        {!projectPath && (
          <View style={{ backgroundColor: c.card, borderRadius: CornerRadius.large, padding: Spacing.lg }}>
            <Text style={[Typography.footnote, { color: c.textSecondary }]}>
              Agent not found or no project path
            </Text>
          </View>
        )}
